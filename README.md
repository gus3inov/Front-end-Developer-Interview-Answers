...
##### :question: Q - Что такое лексическая область видимости ?
###### :heavy_check_mark: A: 

Лексическая область видимости - это область видимости, которая основана на том, где переменные и блоки области видимости были созданы во время написания кода.

[Ссылка на ресурс](https://github.com/azat-io/you-dont-know-js-ru/blob/master/scope%20%26%20closures/ch2.md)
___

##### :question: Q - Объясните, как this работает в JavaScript.
###### :heavy_check_mark: A: 
`this` - это привязка контекста к функции, которая создается во время вызова функции, и на что она ссылается определяется тем, где и при каких условиях функция была вызвана. При каждом вызове функции создается новый `this`
Привязка бывает разных видов:
 - Привязка по умолчанию: происходит при прямом вызове функции, которая не обернута ссылкой на функцию.`undefined` в режиме `strict mode`, в противном случае объект `global`.
...
##### :question: Q - Расскажите, как работает прототипное наследование.
###### :heavy_check_mark: A: 
Механизмы JavaScript могут напоминать "создание экземпляров классов" и "наследование классов" из традиционных класс-ориентированных языков, ключевое отличие в том, что в JavaScript не создаются копии. Вместо этого объекты связываются друг с другом через внутреннюю цепочку [[Prototype]].
В общем случае прототипное наследование реализуется следующим способом: с помощью `Object.create(...)` создаётся "новый" объект, и связывает внутреннюю ссылку прототипа этого объекта с указанным объектом. Но на самом деле термин "наследование", не имеют в JavaScript никакого смысла, учитывая то как это работает. Подходящим термином является "делегирование", поскольку эти связи являются не копиями, а делегирующими ссылками.

[Ссылка на ресурс](https://github.com/azat-io/you-dont-know-js-ru/blob/master/this%20%26%20object%20prototypes/ch5.md)
___
...
###### :heavy_check_mark: A: 
В этом примере будет выброшено исключение `SyntaxError`, в данном случае это объявление функции и скобки в конце не будут отнесены к функции, так как они играют роль оператора группировки. Такие скобки не могут быть пустыми, поэтому будет выброшено исключение.
...
###### :heavy_check_mark: A: 
Прежде чем движок JavaScript начнет интерпретировать код, он его скомпилирует. Один из процессов компиляции является нахождение и ассоциация всех объявлений с их соответствующими областями видимости. 
...
###### :heavy_check_mark: A: 
 ...
 - В случае feature inference мы так же определяем наличие функции у браузера, но в зависимости от проверки используем другую функциональность, как бы предпологая что она так же будет существовать
 - Проверка строки user agent является старой практикой. user agent браузера содержит название и версию приложения, а также данные об операционной системе компьютера: версия, разрядность, язык по умолчанию и другие параметры.
 ___
...
Недостатки:
 ...
 - усложнение проекта путем переноса логики по форматированию данных на клиент
...
###### :heavy_check_mark: A:
На этот счет бытуют разные мнения. Многие предполагают что расширение стандартных объектов вполне допустимо, но перед тем как создать метод, нужно проверить его на существование, что бы случайно не затереть уже имеющийся метод.
 ___
...
##### :question: Q - Что такое цикл событий (event loop) ? 
...
**macrotasks**: setTimeout, setInterval, setImmediate, requestAnimationFrame, I/O, рендеринг пользовательского интерфейса 
